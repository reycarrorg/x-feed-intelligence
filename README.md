...
2. a conditional browser-extension collector that observes visible posts only while the user browses an explicitly approved X origin; optional, bounded hands-free scrolling is separately enabled in Settings.

The recording path remains the lower-account-risk default. The Gate 3 extension uses a dedicated live-DOM v2 schema, popup controls plus a count-only page bubble, manual or opt-in bounded assisted scrolling, hidden-tab pause, and hard stops. Each capture part remains below the 10,000-card/15-MiB export limits and the 9-MiB refresh-recovery limit; a verified automatic save starts the next part. The 8-hour duration applies per part. Continuous parts are retained; only standalone manual exports use the ten-file recycling rule. A narrow extension background context handles local downloads; it does not fetch X or other network data. There is no X API access, network interception, cookie access, link opening, media download, or account-action capability. An earlier review build was locally installed and smoke-tested, but this updated candidate has not been tested in the user's Firefox client and is not signed or published; full authenticated-X acceptance, measurement against a reference recording, and account-safety assessment remain incomplete. Passive extraction may still violate X's Terms and could lead to account enforcement.
