...
- `src/xfi/validation.py` implements strict UTF-8 JSON parsing, byte, depth, version, schema, semantic, digest, duplicate-key/ID, finite-number, Unicode, relationship/provenance-reference, secret, private-surface, and resource validation before persistence. Errors expose category codes only.
- `src/xfi/canonical.py` implements canonical UTF-8 JSON, SHA-256 digests, permalink normalization, relation-aware author selection, generated relationship targets, conflict gates, conservative cross-modality reconciliation, temporal review eligibility, and deterministic deduplication.
- `src/xfi/store.py` owns SQLite files, migrations, application/schema identity, foreign keys, secure deletion configuration, WAL, idempotent transactional imports, a transactionally rebuilt canonical index, parameterized persistence, auditable review decisions, export/backup inventory, online backup, isolated restore, scoped temporary cleanup, checkpointing, purge verification, and honest vacuum outcomes.
...
- 20 Gate 2 core/store/render tests;
- 6 browser-harness tests, including one real MV3 launch;
- 3 Apple Vision recording tests;
- 3 shared-corpus integration tests;
- 3 end-to-end operator CLI tests;
...
The actual browser output—not a hand-shaped surrogate—passed the shared v1 validator and digest check, imported transactionally into the shared SQLite store, canonicalized to 4 posts, and produced 2 organic analyses; the promoted and ambiguous units remained outside organic analysis. The same store then accepted the Apple Vision envelope for the same four identities, retained both modalities and their uncertainty records, emitted the quote edge to the exact generated source ID, and produced byte-identical canonical state regardless of import order. Purging either session transactionally rebuilt canonical records only from surviving observations, removing every purged observation/provenance ID while retaining the other modality. A deliberately contradictory numeric invariant still rolled back. The browser run used a fresh temporary profile, exact reserved-host HTTPS mapping, and a deny-by-default proxy. It allowed only reserved-host tunnels and denied every other browser request. No ordinary or authenticated profile was opened.

The browser harness also accepted a complete normalized envelope at its exact measured UTF-8 length and returned no packet at one byte above the configured limit, entering `ERROR` with `PACKET_LIMIT`. The local proxy tracks client and upstream socket errors, handles resets, and destroys remaining sockets before deterministic server closure.

After the full opt-in suite, the isolated runner passed three additional consecutive fresh-profile launches. All three accepted the same 5,608-byte envelope at an exact 5,608-byte limit, returned no packet when that envelope was one byte over a 5,607-byte limit, and completed with zero observed connection resets and zero other socket errors. The capability-denied VM separately accepted 5,242,880 bytes and rejected 5,242,881 bytes against the normative maximum.
