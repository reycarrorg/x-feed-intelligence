...
The production extension proposal remains an unexecuted static inspection artifact and is not packaged. The runnable browser harness remains test-only and non-distributable outside this package. `package.json`, `.npmrc`, `pnpm-lock.yaml`, `node_modules`, Playwright Core, the Chrome archive/runtime, and the browser runner are mechanically excluded. The SBOM lists them with `excluded` scope so the adoption is visible without implying product inclusion. There is no live-origin permission, browser binary, remote code, network/model API, native messaging, localhost service, telemetry, signing, notarization, or release claim.
