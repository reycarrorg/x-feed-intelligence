...
- The current review build stops at 10,000 distinct visible cards, 8 hours, 15 MiB, excessive ambiguity, login/challenge/rate-limit surfaces, or prompt-injection-like content. While a live Firefox tab is active, it also keeps an in-memory, refresh-recovery snapshot in `storage.session`; that snapshot has a conservative 9-MiB ceiling. The collector stops before exceeding that recovery ceiling and asks the user to export, rather than silently continuing with a capture that would be lost on refresh. This is not browser-restart persistence. Older 5-MiB and 128-MiB review packets remain importable.
